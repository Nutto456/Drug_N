...
async def get_ai_interaction_analysis(drug1: str, drug2: str, description_en: str) -> dict:
    ...
    system_prompt = (
        ...
        "สำหรับ key 'explanation_th' ให้แปลและสรุปข้อมูลทางเทคนิคเป็นภาษาไทยที่คนทั่วไปเข้าใจง่าย โดยเน้นที่กลไกและความอันตราย "
...
def translate_to_english(text: str) -> str:
    return GoogleTranslator(source='th', target='en').translate(text) if is_thai_text(text) else text
...
def fuzzy_match_drug(drug_name: str, threshold: int = 80) -> str:
    ...
    matches = process.extract(clean_drug, all_drugs, scorer=fuzz.partial_token_set_ratio, limit=1)
    return matches[0][0] if matches and matches[0][1] >= threshold else drug_name
...
async def search_drugs(request: DrugSearchRequest):
    ...
    matches = process.extract(request.query, all_drugs_list, scorer=fuzz.partial_token_set_ratio, limit=10)
...
async def check_interactions(request: DrugListRequest):
    ...
    for drug1, drug2 in itertools.combinations(normalized_drugs, 2):
        row = df[((df["Drug 1"].str.lower() == drug1.lower()) & (df["Drug 2"].str.lower() == drug2.lower())) | ((df["Drug 1"].str.lower() == drug2.lower()) & (df["Drug 2"].str.lower() == drug1.lower()))]
        if not row.empty:
            interaction = row.iloc[0]
            interaction_desc_en = interaction["Interaction Description"]
            display_drug1 = interaction["Drug 1"]
            display_drug2 = interaction["Drug 2"]
            
            ai_analysis = await get_ai_interaction_analysis(display_drug1, display_drug2, interaction_desc_en)
            ...
            interactions.append({
                ...
                "drug2": display_drug2, 
                "severity": severity,
                "severity_th": severity_translations.get(severity, "ไม่รุนแรง"),
